(() => {
  ...
  const events = {
    ...
    winterChristmas: {
      date: "19–23 December 2026",
      venue: "Poole Yacht Club",
      price: "",
      title: "Winter Training Series - Christmas Camp",
      intro: "A five-day winter training block at Poole Yacht Club, continuing the technical and race-development work from the autumn series.",
      focus: "A structured winter block for ILCA 4 and ILCA 6 sailors, with the detailed daily focus shaped around the group and conditions.",
      expect: ["High-quality winter water time", "Technical and racecraft development", "Video analysis", "Individual feedback"],
      deadline: "Booking interest is now open. Full camp and pricing information will follow."
    },
    winterNY1: {
      date: "27–30 December 2026",
      venue: "Poole Yacht Club",
      price: "",
      title: "Winter Training Series - New Year's Camp #1",
      intro: "Four days of focused training between Christmas and New Year at Poole Yacht Club.",
      focus: "Maintain momentum through the holiday period with purposeful technical work, racing exercises and winter development.",
      expect: ["Four-day training block", "Technical repetition", "Racecraft exercises", "Video analysis and feedback"],
      deadline: "Booking interest is now open. Full camp and pricing information will follow."
    },
    winterNY2: {
      date: "31 December 2026–3 January 2027",
      venue: "Poole Yacht Club",
      price: "",
      title: "Winter Training Series - New Year's Camp #2",
      intro: "A second four-day New Year training block at Poole Yacht Club, carrying winter development into 2027.",
      focus: "Technical development and race execution with a programme adapted to the sailors attending and the conditions.",
      expect: ["Four-day training block", "Speed and boat-handling work", "Race scenarios", "Video analysis and feedback"],
      deadline: "Booking interest is now open. Full camp and pricing information will follow."
    },
    winter9: {
      ...
      venue: "Poole Yacht Club",
      price: "",
      title: "Winter Training Series #9",
      intro: "A focused weekend block at Poole Yacht Club as the winter programme moves towards the spring season.",
      focus: "Consolidating winter gains through technical training, starts and race-focused exercises.",
      expect: ["Two-day training block", "Technical development", "Starting and racecraft work", "Video analysis and feedback"],
      deadline: "Booking interest is now open. Full camp and pricing information will follow."
    },
    winterFebruary: {
      ...
      venue: "Poole Yacht Club",
      price: "",
      title: "Winter Training Series - February Training Camp",
      intro: "A five-day February training camp at Poole Yacht Club for a concentrated mid-winter development block.",
      focus: "A varied programme combining technical work, speed, starts and racecraft across a full training week.",
      expect: ["Five-day training block", "Speed and technique", "Starts and racecraft", "Video analysis and feedback"],
      deadline: "Booking interest is now open. Full camp and pricing information will follow."
    },
    winter10: {
      ...
      venue: "Poole Yacht Club",
      price: "",
      title: "Winter Training Series #10",
      intro: "The final listed winter weekend at Poole Yacht Club, following directly on from the February Training Camp.",
      focus: "Consolidate the week’s learning and sharpen race execution ahead of the spring season.",
      expect: ["Two-day training block", "Technical consolidation", "Race-focused training", "Video analysis and feedback"],
      deadline: "Booking interest is now open. Full camp and pricing information will follow."
    }
  };
  ...
  if (heroImage) {
    heroImage.src = "assets/images/winter-training.jpg";
    heroImage.alt = "ILCA sailor training at speed on the water";
  }
  ...
  if (list && !document.querySelector('[data-event="winterChristmas"]')) {
    ...
    winterCards.forEach(([key, month, date, title, subtitle]) => {
      ...
      article.innerHTML = `
        <button class="event-trigger" type="button" aria-expanded="false" aria-controls="detail-${key}">
          <span class="event-date"><small>${month}</small>${date}</span>
          <span class="event-main"><strong>${title}</strong><small>Poole Yacht Club · ${subtitle}</small><span class="event-availability"><small>ILCA 4 <strong>Spaces available</strong></small><small>ILCA 6 <strong>Spaces available</strong></small></span></span>
          <span class="event-price winter-new-pill">NEW</span><span class="event-arrow" aria-hidden="true">→</span>
        </button>
        <div class="mobile-event-detail" id="detail-${key}"></div>`;
  ...
  injectedStyles.textContent = `
    .winter-date-divider { display:flex; align-items:center; justify-content:space-between; gap:16px; margin:26px 0 4px; padding:13px 16px; border-left:4px solid var(--blue-600); background:var(--ice); text-transform:uppercase; }
    .winter-date-divider span { font-size:.78rem; font-weight:850; letter-spacing:.08em; }
    .winter-date-divider strong { color:var(--blue-600); font-size:.72rem; letter-spacing:.08em; }
    .winter-event-card { border-color:#bfd0ee; }
    .winter-new-pill { min-width:44px; padding:6px 8px; border-radius:999px; color:var(--white); background:var(--blue-600); font-size:.66rem; line-height:1; text-align:center; }
    .event-availability { display:flex; flex-wrap:wrap; gap:5px 14px; margin-top:7px; }
    .event-availability small { font-size:.69rem; }
    .event-availability strong { font-weight:850; }
    @media (max-width:860px) {
      .event-card.is-selected .event-availability,
      .event-card.is-selected .event-availability small,
      .event-card.is-selected .event-availability strong { color:var(--ink) !important; }
    }
    @media (max-width:620px) {
      .winter-date-divider { align-items:flex-start; flex-direction:column; gap:2px; }
      .event-availability { display:grid; gap:2px; }
      .winter-new-pill { min-width:38px; font-size:.58rem; }
    }
  `;
  document.head.appendChild(injectedStyles);

  const detailMarkup = (event, includeTitle = true) => `
    <div class="camp-topline"><span>${event.date}</span>${event.price ? `<strong>${event.price}</strong>` : `<strong class="new-date-label">NEW DATE</strong>`}</div>
  ...
  function selectEvent(card) {
    ...
    cards.forEach(item => {
      ...
      if (trigger) trigger.setAttribute("aria-expanded", String(selected));
      if (mobileDetail) mobileDetail.innerHTML = selected ? detailMarkup(event, false) : "";
    });
  ...
  if (firstAvailableCard) selectEvent(firstAvailableCard);
